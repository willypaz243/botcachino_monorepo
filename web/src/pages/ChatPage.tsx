...
export const ChatPage: React.FC = () => {
  ...
  const inputRef: React.MutableRefObject<HTMLTextAreaElement | null> =
    useRef(null);
  ...
  const persistMessages: (botMessageId: string) => Promise<void> = useCallback(
    async (_botMessageId: string) => {
  ...
  const handleSendMessage: (content: string) => Promise<void> = useCallback(
    async (content: string) => {
      ...
      if (messages.length === 1) {
        const title =
          content.trim().split(/\s+/).slice(0, 5).join(" ") || "Nuevo chat";
  ...
  const handleSelectChat: (chatId: string) => Promise<void> = useCallback(
    async (chatId: string) => {
